...
inline uint16_t byteswap_ushort(uint16_t i) {
#ifdef __linux__
  uint16_t j;
  j = (i << 8);
  j += (i >> 8);
...
inline uint32_t byteswap_ulong(uint32_t i) {
#ifdef __linux__
  uint32_t j;
  j = (i << 24);
  j += (i << 8) & 0x00FF0000;
...
inline uint64_t byteswap_uint64(uint64_t i) {
#ifdef __linux__
  uint64_t j;
  j = (i << 56);
...
__forceinline uint8_t BitScanReverse(uint64_t *const Index,
                                     const uint64_t Mask) {
#ifdef __linux__
...
__forceinline uint8_t BitScanForward(uint64_t *const Index,
                                     const uint64_t Mask) {
#ifdef __linux__
...
__forceinline uint32_t rotl(uint32_t value, int32_t shift) {
#ifdef __linux__
